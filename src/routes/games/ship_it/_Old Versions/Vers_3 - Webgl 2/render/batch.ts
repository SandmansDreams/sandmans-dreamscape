...
// Attribute locations for INSTANCED_2D_VERTEX_SOURCE. These are a separate
// scheme from mesh.ts's ATTR_* - different program, different VAO, so the
// overlapping numbers never meet.
export const INST_VERTEX = 0    // per vertex   - the base geometry
export const INST_OFFSET = 1    // per instance - world position
export const INST_ROTATION = 2  // per instance - (cos, sin) * scale
export const INST_COLOR = 3     // per instance - rgb

export interface VertexAttribute {
    location: number
    size: number // floats: 2 for a vec2, 3 for a vec3
}

/** A 1x1 square centred on the origin, so the instance transform scales about its middle. */
...
export class InstancedBatch {
    ...
    constructor(
        ...
        Assert.exists(vertexArray, "vertexArray")
        this.vertexArray = vertexArray
        gl2.bindVertexArray(vertexArray)

        // Base geometry: static, advances per vertex.
        const baseBuffer = gl2.createBuffer()
        Assert.exists(baseBuffer, "baseBuffer")
        this.baseBuffer = baseBuffer
        gl2.bindBuffer(gl2.ARRAY_BUFFER, baseBuffer)
        ...
        const instanceBuffer = gl2.createBuffer()
        Assert.exists(instanceBuffer, "instanceBuffer")
        this.instanceBuffer = instanceBuffer
        gl2.bindBuffer(gl2.ARRAY_BUFFER, instanceBuffer)
