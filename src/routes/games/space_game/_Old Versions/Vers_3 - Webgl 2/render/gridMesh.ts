...
export function buildGridMesh(
    ...
    for (const cell of grid.list) {
        ...
        if (!withCellCentre) {
            appendShape(out, cell.shape, cell.turns, cell.mirrored, x, y, cellSize, ...cell.color)
            continue
        }

        vertices.length = 0
        appendShape(vertices, cell.shape, cell.turns, cell.mirrored, x, y, cellSize, ...cell.color)
