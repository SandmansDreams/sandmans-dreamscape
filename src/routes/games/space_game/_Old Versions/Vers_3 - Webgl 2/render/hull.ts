...
export function saveHull(grid: Grid, palette: Record<string, RGB> = {}): HullData {
    ...
    for (const cell of grid.list) {
        const match = named.find(([, rgb]) =>
            rgb[0] === cell.color[0] && rgb[1] === cell.color[1] && rgb[2] === cell.color[2]
        )
        ...
        if (match) {
            ...
            key = `color${generated++}`
            named.push([key, [cell.color[0], cell.color[1], cell.color[2]]])
        }

        used[key] = used[key] ?? [cell.color[0], cell.color[1], cell.color[2]]
