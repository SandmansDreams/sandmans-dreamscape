...
export class Camera {
    ...
    constructor(position: Vec2 = { x: 0, y: 0 }, zoom = 1, rotation = 0) {
    ...
    screenToWorld(screenX: number, screenY: number): Vec2 { // Converts screen coordinates to world coordinates for input handling
        const zoom = this.zoom > 0 ? this.zoom : 1
    ...
    worldToScreen(worldX: number, worldY: number): Vec2 { // Converts world coordinates to screen coordinates for interactive elements
        const zoom = this.zoom > 0 ? this.zoom : 1
...
export class CameraBinding {
    ...
    static create(gpu: GPU, label = "camera"): CameraBinding {
        const layout = gpu.device.createBindGroupLayout({
            label: `${label} layout`,
            entries: [{ binding: 0, visibility: GPUShaderStage.VERTEX, buffer: { type: "uniform" } }],
        })

        const buffer = Buffer.makeUniformBuffer(gpu, CAMERA_BYTES, `${label} buffer`)
